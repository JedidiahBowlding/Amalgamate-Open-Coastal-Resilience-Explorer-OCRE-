...
app = FastAPI(title="CORA Annapolis Pilot API", version="0.1.0")
...
def get_annapolis_summary() -> dict:
    if not MVP_PATH.exists():
        raise HTTPException(status_code=404, detail="MVP payload not found. Run extract_cora_data.py first.")
...
def get_annapolis_timeseries() -> list[dict]:
    if not TIMESERIES_PATH.exists():
        raise HTTPException(status_code=404, detail="Time series not found. Run extract_cora_data.py first.")
